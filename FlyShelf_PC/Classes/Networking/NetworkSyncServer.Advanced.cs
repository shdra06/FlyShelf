        ...
        private async Task HandleChunkFinalize(HttpListenerRequest req, HttpListenerResponse res)
        {
            ...
            try
            {
                ...
                if (!LicenseManager.IsPro)
                {
                    ...
                    if (totalChunkBytes > LicenseManager.FREE_SYNC_SIZE_LIMIT)
                    {
                        try { Directory.Delete(chunkDir, true); } catch { }
                        _chunkSessions.TryRemove(sessionId, out _);

                        res.StatusCode = 413;
                        byte[] preErrBytes = Encoding.UTF8.GetBytes("{\"error\":\"File transfer limited to 50 GB on Free tier.\"}");
                        res.ContentType = "application/json";
                ...
                if (fileInfo.Length > LicenseManager.FREE_SYNC_SIZE_LIMIT && !LicenseManager.IsPro)
                {
                    ...
                    res.StatusCode = 413;
                    byte[] errBytes = Encoding.UTF8.GetBytes("{\"error\":\"File transfer limited to 50 GB on Free tier.\"}");
                    res.ContentType = "application/json";
                    await res.OutputStream.WriteAsync(errBytes, 0, errBytes.Length);
