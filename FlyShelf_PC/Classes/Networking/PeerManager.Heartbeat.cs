            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                var pingTasks = alivePeers.Select(async peer =>
                {
                    ...
                    else
                    {
                        ...
                        // Mobile devices: more tolerant (background kills, doze mode)
                        int effectiveMaxFailures = (peer.DeviceType?.Equals("Mobile", StringComparison.OrdinalIgnoreCase) == true
                            || peer.DeviceType?.Equals("Android", StringComparison.OrdinalIgnoreCase) == true)
                            ? MAX_FAILURES + 2  // 5 failures for mobile
                            : MAX_FAILURES;     // 3 failures for PC
                        ...
                        else
                        {
                            ...
                            if (peer.ConsecutiveFailures == 1)
                            {
                                _ = Task.Run(async () =>
                                {
                                    try
                                    {
                                        string altUrl = peer.Transport == "LAN" ? peer.CloudflareUrl : peer.LanUrl;
                                        if (!string.IsNullOrEmpty(altUrl))
                                        {
                                            using var altCts = new CancellationTokenSource(5000);
                                            using var req = new HttpRequestMessage(HttpMethod.Get, $"{altUrl.TrimEnd('/')}/api/health");
