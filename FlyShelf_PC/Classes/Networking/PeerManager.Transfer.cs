    ...
    public partial class PeerManager
    {
                ...
                if (fSize > 5 * 1024 * 1024 && aliveLanPcPeers.Count > 0 && LanTransferManager.Instance != null)
                {
                    ...
                    if (httpPeers.Count > 0)
                    {
                        ...
                        if (cfPeers.Count > 0)
                        {
                            ...
                            {
                                Logger.LogAction("PEER", "Cloudflare transfer limited to 50 GB on Free tier");
                            }
                            else
                            {
        ...
        private async Task<bool> TrySendFile(PeerConnection peer, string filePath, string title, string itemType)
        {
            ...
            finally
            {
                ...
                // FIX: If a URL update was deferred while this transfer was active,
                // reconnect now with the updated URLs to avoid staying on a dead connection.
                if (remaining <= 0 && peer.PendingUrlReconnect)
                {
                    peer.PendingUrlReconnect = false;
                    Logger.LogAction("PEER", $"🔄 Deferred URL reconnect triggered for {peer.DeviceName} — transfer completed");
                    _ = Task.Run(() => Handshake(peer));
                }
            }
        }
