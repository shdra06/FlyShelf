...
namespace FlyShelf.Classes
{
    ...
    public static class LicenseManager
    {
        ...
        private static async Task<(string token, string error)> ActivateOnServerAsync(string key, string deviceId)
        {
            ...
            {
                // Timeout — distinct error so callers can differentiate from network failure
                return (null, "timeout");
            }
            catch (HttpRequestException)
            {
                // Network failure — distinct error so callers can differentiate from timeout
                    ...
                    if (File.Exists(backupKeyPath))
                    {
                        try
                        {
                            ...
                            if (!string.IsNullOrEmpty(backupKey))
                            {
                                ...
                                // PH-5: Clear ActivationToken and LastValidated so the next
                                // RevalidateLicenseAsync() call forces a server check.
                                // Without this, a restored backup key would be trusted without
                                // server validation, bypassing revocation/device-limit checks.
                                _data = new LicenseData
                                {
                                    LicenseKey = backupKey,
                                    Tier = "pro",
                                    ActivationToken = "",
                                    LastValidated = ""
                                };
        ...
        private static async Task ValidateKeyOnServerAsync(string key, string deviceId)
        {
            try
            {
                ...
                if (devicesResponse.IsSuccessStatusCode)
                {
                    ...
                    // PM-16: Use JsonDocument to count actual device properties instead of
                    // fragile Split("true") which can miscount if values contain the word "true".
                    int deviceCount = 0;
                    try
                    {
                        using var devDoc = JsonDocument.Parse(devicesJson);
                        if (devDoc.RootElement.ValueKind == JsonValueKind.Object)
                        {
                            foreach (var _ in devDoc.RootElement.EnumerateObject())
                                deviceCount++;
                        }
                    }
                    catch (JsonException)
                    {
                        Logger.LogAction("LICENSE_SERVER", $"Failed to parse devices JSON: {devicesJson}");
                    }
