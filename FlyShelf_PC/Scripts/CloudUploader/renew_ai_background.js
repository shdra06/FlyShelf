...
(async () => {
    try {
        ...
        const context = await chromium.launchPersistentContext(profilePath, {
            headless: true, // Execute completely silently in the background
            channel: 'chrome',
            viewport: null,
        ...
        console.log("[ADVANCECLIP] Navigating to Billing Portal...");
        try {
            await page.goto("https://admin.google.com/ac/billing/subscriptions", { waitUntil: 'domcontentloaded', timeout: 25000 });
        } catch (e) {
            console.log("[ADVANCECLIP] Safety Lock: Network took too long! Initiating Emergency Reload...");
            await page.reload({ waitUntil: 'domcontentloaded', timeout: 30000 });
        }
        
        // Target: "AI Ultra" Subscription Link
        await page.getByText(/AI Ultra/i).first().waitFor({ state: 'visible', timeout: 30000 });
        ...
        const buyNowFallback = page.locator('span.gws-button a, a[data-g-action="buy now"], a.button:has-text("Buy now"), button:has-text("Buy now")');
        await buyNowFallback.first().waitFor({ state: 'attached', timeout: 30000 });
        await buyNowFallback.first().click({ force: true, timeout: 10000 });
        ...
        // ==========================================
        // POPUP MANAGEMENT PIPELINE
        // ==========================================
        // Automatically kill any external popups trying to spawn over the host natively!
        context.on('page', async newPage => { 
            console.log("[ADVANCECLIP] Sensed an external Popup Window! Closing it forcefully to maintain execution focus.");
            await newPage.close(); 
        });

        console.log("[ADVANCECLIP] Handling 'Add funds to account' OK Confirmation...");
        // Fallback: If it's an internal DOM Modal, click OK:
        const okModalBtn = page.getByRole('button', { name: /^(OK|Ok)$/ }).first();
        try {
            await okModalBtn.waitFor({ state: 'visible', timeout: 10000 });
            await okModalBtn.click();
        } catch { console.log("[ADVANCECLIP] No Internal DOM OK Modal required."); }

        // Final Payment Cooldown
        await page.waitForTimeout(10000);

        // Verification Target
        console.log("[ADVANCECLIP] Navigating exact Payment Access Link...");
        const paymentAccountPage = page.getByRole('link', { name: /Payment account page/i }).first();
        await paymentAccountPage.waitFor({ state: 'visible', timeout: 15000 });
        await paymentAccountPage.click();

        console.log("\n[ADVANCECLIP] SUCCESS - Full Sequence Completed Elegantly!");
